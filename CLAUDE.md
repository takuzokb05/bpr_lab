...
**現状把握はまずルートの `STATUS.md` を読むこと**（リポジトリ全体マップ・サブプロジェクトINDEX・直近の変更）。ふわっとした指示を受けたときの起点もここ。
...
収集記事・カタログ・ダイジェスト等の library 資産は**必ず `sandbox/タスクマネージャー/library/` 配下**に置く。リポジトリルート直下に `library/` を作らない（2026-05-27〜07-15 に相対パス解決の誤りでルートに分裂蓄積した事故があり、07-16 に統合済み。ルート `library/MOVED.md` 参照）。
