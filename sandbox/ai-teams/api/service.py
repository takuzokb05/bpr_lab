...
# .env を読み込んでおく（ANTHROPIC_API_KEY / AI_COUNCIL_MODEL 等）。**core を import する前に**
...
from core.llm_client import _env  # noqa: E402 — .env を先に読むため
...
def persona_public(p: Persona) -> dict:
    ...
    return {
        ...
        "category": p.category,
        # ピッカーで名前の下に出す一行ティーザー（「どんな人か」）。空文字なら UI 側で非表示。
        "description": p.description,
        # 「詳細」展開で出す詳しい説明（偉人の背景＋持ち味 等）。空文字なら UI 側で非表示。
        "detail": p.detail,
...
def byok_mode() -> bool:
    ...
    共有/公開インスタンスでは AI_COUNCIL_BYOK=1 を設定する。すると make_client は
    **サーバの ANTHROPIC_API_KEY を来訪者リクエストに一切使わず**、リクエスト毎に
    ...
    """
    return _env("BYOK", "").strip().lower() in ("1", "true", "yes")


def readonly_mode() -> bool:
    """編成 CRUD（ペルソナ/プリセットの作成・更新・削除）を禁止するか。

    共有インスタンスでは AI_COUNCIL_READONLY=1 を設定し、来訪者がサーバ上のファイルを
    書き換え/量産/削除できないようにする（main.py 側で 403 にマップ）。
    """
    return _env("READONLY", "").strip().lower() in ("1", "true", "yes")
...
def local_base_url() -> str:
    ...
    env AI_COUNCIL_LOCAL_BASE_URL（例: http://127.0.0.1:11434/v1 ＝ Ollama、http://host:8000/v1 ＝ vLLM）。
    未設定なら空（＝内製は無効）。推論サーバは uvicorn と別ホストでよい（base_url で繋ぐだけ）。
    """
    return _env("LOCAL_BASE_URL", "").strip()


def local_model() -> str:
    """内製 LLM のモデル名（env AI_COUNCIL_LOCAL_MODEL、例: qwen3:14b）。未設定なら汎用 'qwen3'。"""
    return _env("LOCAL_MODEL", "").strip() or "qwen3"
...
# （所有者の OpenRouter キーで高額モデルを勝手に使わせない）。env AI_COUNCIL_LOCAL_PRESETS で
...
def local_presets() -> list[dict]:
    """討論モードのプリセット一覧。env AI_COUNCIL_LOCAL_PRESETS(JSON) があればそれを、無ければ既定。"""
    raw = _env("LOCAL_PRESETS", "").strip()
    if raw:
        try:
            data = json.loads(raw)
...
def force_local() -> bool:
    """サーバを丸ごと内製（local）に固定するか（env AI_COUNCIL_FORCE_LOCAL）。
    ...
    """
    flag = _env("FORCE_LOCAL", "").strip().lower() in ("1", "true", "yes")
...
# Web 検索の「新規クエリ」合計上限（暴走防止）。env AI_COUNCIL_RESEARCH_CAP で可変（既定 12）。
# 旧既定 6 は中規模討論の中盤で枯れて「要調査が無視される」事故になった。dedup 済みの新規クエリ
# だけを数えるので、同じ問いの再検索は数に入らない。1検索 ≒ $0.005 なので 12 でも安い。
_RESEARCH_CAP = int(_env("RESEARCH_CAP", "12"))
...
# env AI_COUNCIL_RESEARCH_PER_TURN で可変（既定 2）。序盤に検索が殺到するのを抑える。
_RESEARCH_PER_TURN = int(_env("RESEARCH_PER_TURN", "2"))
...
def build_council(
    ...
    *,
    ...
    # env AI_COUNCIL_SYNTHESIS_MAX_TOKENS で下限を可変（既定 8192）。非数値は既定にフォールバック。
    try:
        synth_floor = int(_env("SYNTHESIS_MAX_TOKENS", "8192"))
    except ValueError:
        synth_floor = 8192
...
# env AI_COUNCIL_MAX_ACTIVE で可変（既定 12）。超過時の生成は CapacityError → 429。
MAX_ACTIVE_SESSIONS = int(_env("MAX_ACTIVE", "12"))
...
# env AI_COUNCIL_FLOOR_IDLE_TIMEOUT（既定 1800=30分。0 で無制限＝従来動作）。
_FLOOR_IDLE_TIMEOUT = float(_env("FLOOR_IDLE_TIMEOUT", "1800"))
