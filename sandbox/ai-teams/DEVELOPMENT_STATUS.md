...
- **全ペルソナに一行説明（description）**: 「この人はどんな人か」をピッカーで名前の下に表示し、説明も検索対象に。
  データモデル全層に `description` を追加 — `core/personas.py`（dataclass＋ローダ）／`api/service.py`
  （`persona_public`＋`_PERSONA_WRITE_KEYS`）／**`api/main.py` の `PersonaUpsert`・`CustomPersona`（Pydantic 受け口）**／
  `web/lib/types.ts`／`PersonaPicker`（line-clamp-2 表示）／`PersonaForm`・`MyPersonasDrawer`（入力欄）。
  `personas/**/*.yaml` 46体すべてに一行説明を付与（各 system_prompt を読み文体・長さを揃えて生成）。
- **対抗レビューで critical 1件を捕捉・修正**: `PersonaUpsert`（Pydantic）に `description` が無く extra='ignore' で
  黙って捨てられ、全置換 save により**既存ペルソナの編集保存で YAML の description が消える**破壊的バグを是正。
  `relationships` と同じ罠。サービス直叩きの既存テストでは検出不能だったため、**HTTP 層を通る round-trip
  回帰テスト**を `tests/run_tests.py` に追加。reduced-motion ガード・「最新へ」タップ領域・検索 a11y も改善。
- 検証: 全テスト pass／`next build` 通過／`GET /personas` が46体を description 付きで返すことを確認。
