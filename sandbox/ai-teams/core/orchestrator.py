...
# 裁定（verdict）: 議事録の後に議長が「依頼者への答え」を言い切るターン。
# 議事録は中立な記録（対立点を先頭に残す）、裁定は意思決定（割れたままでも答えを出す）と分掌する。
# このツールの至上命題は依頼者の課題解決であり、「ここが割れました」で終わらせない最後の砦。
# 残った対立は「依頼者の状況次第で結論が変わる分岐条件」に変換して、平行線を意思決定ルールにする。
VERDICT_DIRECTIVE = (
    "【裁定】議事録はまとまりました。最後に、あなたはこの会議の議長として、依頼者の議題に"
    "裁定を下してください。討論が割れたまま終わっても、依頼者が今日動けるようにするのが"
    "裁定の役目です。必ず次の順序の見出しで簡潔に:\n"
    "1. 結論 — 議題への直接の答えを1〜3文で言い切る。両論併記や『状況による』への逃げは禁止。"
    "討論全体で最も説得力のあった一手を、議長の責任で選ぶこと\n"
    "2. 決め手 — なぜその結論か。どの論点・どの発言が判断を分けたか\n"
    "3. 結論が変わる条件 — 依頼者の状況次第で結論が覆るなら『〜ならば、代わりに〜』の形で"
    "最大2つ（残った対立点を、依頼者が自分で判定できる分岐条件に変換する）\n"
    "4. 残る反対意見 — この裁定に最後まで反対する登壇者と理由を1行だけ（消さずに残す）\n"
    "新しい論点は足さず、討論で出た議論（【資料・前提】があれば依頼者の制約・成功の定義も）"
    "だけから裁定すること。"
)
...
DEFAULT_PHASES: list[tuple[str, str, bool]] = [
    ...
    (
        "収束",
        "【収束フェーズ】司会がまとめた合意点は**繰り返さない**こと。ここではあなたの"
        "**最終立場**を確定させてください。次の2点だけを簡潔に:\n"
        "(1) 対立した相手の論点のうち最も強かったもの1つを名指しで認める"
        "（どこまでなら受け入れられるかも一言）。\n"
        "(2) それを踏まえた最終推奨を**条件付き**で述べる——『〜という条件なら私の案、"
        "〜なら相手の案が正しい』の形で、あなたの判断が覆る具体的な条件を必ず1つ明示する。"
        "条件を示せないほど確信があるなら、その根拠を1つに絞って言い切ること。"
        "既出の合意や他者の発言の再掲・言い換えは禁止。",
        False,
    ),
]
...
class Council:
    ...
    def deliberate(
        ...
        *,
        ...
        for phase_name, directive, anti in self.phases:
            # 収束の口火（司会・在席時のみ）: 合意点を**1回だけ**まとめた上で、最も深い対立軸を
            # 1つ特定し、割れの源泉（事実認識/価値観/前提）を診断する（争点診断）。これが無いと
            # 各自の収束発言が別々の論点に散り、平行線のまますれ違う。合意の再掲冗長の抑止も兼ねる。
            if phase_name == "収束" and self.moderator is not None:
                turn = self._speak(
                    ...
                    round_no=0,
                    phase_directive=(
                        "【収束の口火（司会・争点診断）】まず、ここまでの議論で固まった合意点を1文で"
                        "まとめてください。次に、最も深く割れている対立軸を**1つだけ**特定し、"
                        "『誰と誰が、何について割れているか』を1文で示すこと。さらにその割れの源泉が "
                        "(a)事実認識の違い (b)価値観・優先順位の違い (c)前提・状況想定の違い "
                        "のどれかを診断して言い添えること。そのうえで各登壇者へ、『すでに合意した点は"
                        "繰り返さず、この対立軸への最終立場と、自分の判断が覆る条件』を述べるよう"
                        "促してください。あなた自身は結論を出さないこと。"
                    ),
                    anti_conformity=False,
    ...
    def synthesize(
        ...
        *,
        ...
        """議長による統合（議事録）→ 裁定（verdict）の2ターンを yield する。chair 不在なら司会が兼任。

        かつて 3行エグゼクティブサマリ(summary フェーズ)を別に出していたが、実 LLM が
        3行指示を守らず議事録と重複した上、短すぎて読まれないため廃止。議事録1枚に集約する。

        裁定（phase="verdict"）は議事録の直後に出す: 議事録＝中立な記録（対立点先頭）、
        裁定＝依頼者への答え（割れたままでも結論を言い切り、残る対立は分岐条件に変換）。
        議事録を先に生成することで、裁定は構造化済みの議事録を文脈に踏まえられる。
        ...
        turn = self._speak(
            ...
            anti_conformity=False,
            research_override=False,  # 統合は枠組みターン＝要調査を誘発しない（close 経路は拾わないため無言無視になる）
            emit=emit,
            turn_id=next(ids),
        ...
        # 裁定: 議事録を踏まえ、議長が依頼者の議題に答えを言い切る（平行線を意思決定に変換）。
        # 枠組みターンなので要調査は誘発しない（research_override=False）。
        turn = self._speak(
            synthesizer,
            transcript,
            topic,
            phase="verdict",
            round_no=0,
            phase_directive=VERDICT_DIRECTIVE,
            anti_conformity=False,
            research_override=False,
            emit=emit,
            turn_id=next(ids),
            # 裁定は議事録より短いが、分岐条件まで書くと標準上限では切れうる。同じ上限を使う。
            max_tokens=self.synthesis_max_tokens,
        )
        transcript.append(turn)
        yield turn
