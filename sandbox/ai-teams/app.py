...
2. **【議論の現在地】**
   （不足している視点・未発言者の確認・次に必要なアクションのメタ分析）
...
"""
    ...
    for m in recent_msgs:
         ...
         # 引用なら `> 🎤` となるはずなので `\n🎤` にはマッチしないはず。
         # --- Context Scrubbing (Advanced) ---
         # もし発言者がモデレーターで、かつ次の話者(agent)がメンバーなら、モデレーターの「要約」や「予言」を隠す。
         # 具体的には、モデレーターの発言の「要約」部分を削除するか、警告を追加する。
         if m.get('role') == 'assistant' and m.get('agent_name') and ("モデレーター" in m['agent_name'] or "司会" in m['agent_name']):
               # モデレーターの発言
               # 1. [[NEXT: ID]] タグを除去する（これは上の cln = re.sub... で既にやっている）
               # 2. 【指名】ブロックだけを残し、要約を切り詰めるオプション
               #    しかし、メンバーも「議論の流れ」を知る必要はあるので、完全削除は危険。
               #    代わりに、タグ付けして「これは要約である」と明示するか、あるいは
               #    モデレーターの「意思（Thought）」が含まれていそうな部分を警戒する。
               
               # 現状は、モデレーターの出力が「3ブロック構造」になっているはずなので、
               # 【指名】ブロックが最後にあることを利用し、その手前までを「Context」として渡す。
               # もしモデレーターが「...ですので、次は〇〇さん、A案でお願いします」と指名ブロックに書いていたら汚染される。
               # -> 新プロンプトで「指名ブロックはシンプルに」と指示したので、それを信じる。
               
               pass # ロジックが複雑になりすぎるので、まずは「Hard Stop」による物理遮断を信頼する。
                    # 将来的には xmlタグで <summary>...</summary> と囲むのもあり。
