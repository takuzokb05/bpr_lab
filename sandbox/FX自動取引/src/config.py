...
# 急変への追従性 vs ノイズ耐性のバランス。スプレッドキルの判断材料となるため
# config 化。
SPREAD_EMA_ALPHA: float = 0.1

# 観測性: パイプライン1行サマリログ（trading_loop._log_pipeline_trace）の
# detail フィールド最大長。AI/Bear の reasoning など長文を切り詰めて
# 1行サマリの可読性を保つ。40字 = 1ペア trace が概ね 200字以内に収まる目安。
PIPELINE_TRACE_DETAIL_MAXLEN: int = 40
