...
{{
  ...
  "confidence": 0.50/0.55/0.60/0.65/0.70/0.75/0.80/0.85/0.90 のいずれか,
...
confidence の付け方 (重要):
- 次の 4 観点を個別に評価する: (1) シグナル方向と直近トレンドの整合
  (2) 直近 1h モメンタムの裏付け (3) 関連通貨ペアの動きとの整合
  (4) リスク要因の少なさ (レンジ・荒れ相場・流動性)
- 4 観点すべて強い裏付け → 0.85-0.90 / 3 観点 → 0.75-0.80 /
  2 観点 → 0.65-0.70 / 1 観点以下 → 0.50-0.60
- 毎回同じ値に寄せない。観点ごとの強弱が違えば confidence も必ず変える
"""
