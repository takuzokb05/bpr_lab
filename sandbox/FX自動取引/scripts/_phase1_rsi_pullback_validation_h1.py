"""Phase 2-C 戦略検証: H1 / 730日 で RsiPullback バックテスト + 感度分析

P2-C 用拡張版。M15 60日 (= Phase 1) では OOS Trades が 9〜16 件しかなく
統計的検出力が不足していたため、H1 730日 (~17,000 バー) で再検証する。

対象: USD_JPY, EUR_USD, GBP_JPY (H1, 730日)
出力:
- data/backtest_grid_h1_<pair>.csv
- data/phase2c_validation_summary.json
- 標準出力 (data/_phase2c_run.log にリダイレクト推奨)
"""
...
def main():
    ...
    # ----- 全体サマリ JSON -----
    import json
    sum_path = out_dir / "phase2c_validation_summary.json"
