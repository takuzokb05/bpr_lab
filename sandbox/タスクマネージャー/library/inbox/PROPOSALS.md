...
### 1. Claude Code: SKILL.md 3レベル動的ロード構造の採用

**出典:** articles/2026-05-04_1296_SKILL_md_3Levels_Specialized_Agent_30min_RetroChainer.md

**提案内容（優先度: 高）:**
@RetroChainerの実践によると、SKILL.mdを「必要なものだけロードする」3段階構造にすることで30分タスクが3分に短縮できる。現在のスキルは全情報を1ファイルに詰め込んでいるため、コンテキスト消費が大きい。

**アクション:**
- 現行スキルを「Level 1: 常時ロード（最小限の起動定義）」「Level 2: タスク判断後ロード（手順詳細）」「Level 3: 必要時オンデマンド（参考資料・例示）」の3段階に分割するリファクタリングを検討。
- `.claude/skills/` 配下の大きなスキルから順に段階化を試行する。

### 2. Claude Code + MCP: EC業務外注費94%削減事例の横展開

**出典:** articles/2026-05-04_1295_EC_Outsource_Cost_500k_to_30k_Claude_Code_MCP_ROI.md

**提案内容（優先度: 中）:**
EC運用外注費を月50万→3万円（94%削減）に切り替えた実践例。①MCP経由でスプレッドシートに直接接続して定型レポート自動生成②200SKU分の商品説明をブランドガイドライン参照で一括生成 という2パターンが再現性高い。

**アクション:**
- FX自動取引の週次レポート自動生成にMCP + Claude Code組み合わせを試行。スプレッドシートMCPサーバーで取引ログを直接参照してのレポート生成フロー構築を検討。

### 3. Claude API: キーレス認証（Workload Identity Federation）への移行検討

**出典:** articles/2026-05-04_1303_Claude_API_Keyless_Auth_AWS_IAM_GCP_GitHub_Actions_K8s.md

**提案内容（優先度: 高）:**
Claude PlatformがAWS IAM・GCP・GitHub Actions・k8s・Microsoft Entra ID・Okta対応のWorkload Identity Federationを正式提供開始。長期APIキーの保存が不要になりセキュリティリスクが大幅低下。

**アクション:**
- `sandbox/FX自動取引/.env.example` に記載しているAPIキー管理をGitHub Actions + Workload Identity Federationへ移行する手順を検討。CI/CDパイプラインでの短期トークン動的発行方式への移行ロードマップを作成。
- 公式ドキュメント: https://platform.claude.com/docs/en/build-with-claude/workload-identity-federation

### 4. AI規制リスク: ホワイトハウスAI公開前審査の動向監視

**出典:** articles/2026-05-04_1304_WhiteHouse_AI_PreRelease_Review_Trump_AISI_Type.md / articles/2026-05-04_1313_WhiteHouse_Frontier_AI_Oversight_Before_Release_Major_Policy_Shift.md

**提案内容（優先度: 中・モニタリング）:**
トランプ政権がフロンティアAIの公開前審査（英国AISI型）を大統領令で義務化する動きを検討中。Anthropic・Google・OpenAIとWHが既に協議済み。フロンティアAIの市場投入が「許認可産業化」するリスクがある。

**アクション:**
- FX自動取引に使用するAnthropicモデルの利用可能性に影響する可能性があるため、Anthropic APIの規制対応状況を月次でモニタリングするフローを設定。
- 特にClaude Mythosレベルの強力モデルが規制対象となり利用制限される可能性を念頭に置く。

### 5. FX自動取引: Python→ONNX→EA技術スタックの実装パス確認

**出典:** articles/2026-05-04_1310_AI_Trading_ONNX_EA_Python_Implementation_SakuraBurst.md

**提案内容（優先度: 高）:**
AIモデルのFX EAへの組み込みはPython学習→ONNX変換→MQL5 EAへの組み込みという3段階フローが現実的な実装パス。個人実装には相当な工数がかかることが実践者により報告されている。

**アクション:**
- `sandbox/FX自動取引/docs/04_技術設計と実装方針.md` にONNX変換を利用したMLモデル組み込みアーキテクチャを選択肢として追記。
- Claude Code + MT5 EAのPF 0.73→1.83改善事例（articles/2026-05-04_1309）を参考に、シンプルな統計ベースアプローチから始めてML組み込みへと段階的に発展させるロードマップを検討。

### 6. Anthropic Agentic Stack: ADK + MCP + Vertex AI Agent Engineの活用

**出典:** articles/2026-05-04_1307_Anthropic_Agentic_Stack_ADK_MCP_Vertex_AI_Agent_Engine.md

**提案内容（優先度: 中）:**
Anthropicが「Agentic Stack」として ADK（オープンソース開発ツールキット）+ MCP（標準化コンテキストプロトコル）+ Vertex AI Agent Engine（本番級マネージドプラットフォーム）の3層構成を正式に公開。コードから本番へのフルパスが提供された。
...
### 1. Claude Code Dreaming機能の活用: Self-improving CLAUDE.md との統合

**出典:** articles/2026-05-06_1413_X_ura_unico_Claude_CodeのDreaming機能意外にパワフルです.md / articles/2026-05-06_1459_WEB_AnthropicSpaceX_コンピュート契約Claude_CodeAPI_レ.md

**提案内容（優先度: 高）:**
Code w/ Claude 2026（SF, 5/6）で発表されたClaude Managed AgentsのDreaming機能（過去セッションを振り返り自己改善）とOutcomes（成功条件設定）の組み合わせが実用的。現在のCLAUDE.mdにはSessionEnd Hookによる自動更新提案が未反映（catalog item 14参照）。Dreamingを活用することでCLAUDE.mdの自動進化が現実的になる。

**アクション:**
- `sandbox/タスクマネージャー/.claude/settings.json` にSessionEnd Hookを追加し、会話終了時にDreaming的な振り返りを促すプロンプトを設定。
- Outcomesの成功条件をCLAUDE.mdに定義（例：「タスク完了後に必ずギャップをドキュメント化する」）。

### 2. caveman skill の導入検討: トークンコスト削減

**出典:** articles/2026-05-06_1424_X_chenzeling4_caveman_Claude_Code_skill_that_cuts_75_of_outp.md

**提案内容（優先度: 中）:**
caveman skillはClaude Code出力トークンを75%削減・入力を46%圧縮する。技術的正確性を保ちつつ原始人語スタイルで出力する（55,235 stars・OSS）。長時間実行の情報収集エージェントや大量ファイル処理で効果が大きい。

**アクション:**
- JuliusBrusseeのcaveman skillを `.claude/skills/` に追加し、情報収集エージェント・バッチ処理タスクで試験的に有効化。
- 特に`sandbox/タスクマネージャー/`の日次収集エージェントでトークン削減効果を測定。

### 3. sovra/bash subagent の既知問題: プロジェクト記憶の保護

**出典:** articles/2026-05-06_1427_X_justic_hot_shipped_sovra_in_3_days_with_claude_code_speed_was.md

**提案内容（優先度: 高）:**
長時間実行のbash subagentでコンテキストがドロップする問題と、`/clear`でプロジェクト記憶が全消去される問題は広く報告されている。現在のFX自動取引プロジェクトでも発生リスクがある。

**アクション:**
- CLAUDE.mdに「`/clear`実行前に必ず現在の進捗を`STATUS.md`に書き出すこと」を追記。
- FX自動取引の長時間バックテストタスクでは、サブエージェントごとに独立したコンテキストを持つことを設計に組み込む。
- `/clear`をまたぐプロジェクト記憶として`sandbox/FX自動取引/STATUS.md`を定期更新するHookを追加検討。

### 4. Code w/ Claude 2026 東京（6/10）への参加検討

**出典:** articles/2026-05-06_1472_WEB_Code_with_Claude_2026SFロンドン東京_3都市イベント詳細.md

**提案内容（優先度: 低・参考情報）:**
AnthropicのCode w/ Claude 2026が東京で6月10日に開催予定。Claude Code・Claude Agent SDK・Claude Managed Agentsの最新情報を一次情報として入手できる機会。

**アクション:**
- 東京イベント（6/10）の参加登録を確認。公式ページ: https://claude.com/code-with-claude

### 5. MCP標準化の加速: FX自動取引でのMCP活用を検討

**出典:** articles/2026-05-06_1440_X_aditya_A17_xAI_has_just_rolled_out_Connectors_a_powerful_new.md / articles/2026-05-06_1441_X_lopezunwired_The_AWS_MCP_Server_is_now_generally_available.md

**提案内容（優先度: 中）:**
xAI Connectors（MCP採用）・AWS MCP Server GA（5/6）と、Mistral WorkflowsのMCP採用により、MCPが全主要プレイヤーに普及した。MT5との連携においてMCP経由でブローカーAPIやマーケットデータフィードに接続するアーキテクチャが現実的になってきた。
